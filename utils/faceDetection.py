...
# def face_detector(pixels, margin=0.25):
#     image = pixels
...
#         margin_x = int(bb_width*margin)
#         margin_y = int(bb_height*margin)
...
def face_detector(image):
    img = image.copy()
    # lấy kích thước của ảnh gốc
    base_width, base_height = img.shape[1], img.shape[0]
    # xác định các bbox
    bboxes, scores=face_detector_caffe(image)
    faces_location = []
    for box in bboxes:
        ...
        xmax = int(min(base_width,(box[2] * base_width)))
        ymax = int(min(base_height,(box[3] * base_height)))
        faces_location.append((xmin,ymin,xmax-xmin,ymax-ymin))
